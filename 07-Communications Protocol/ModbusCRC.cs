using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace eTommensDC
{
    //16位CRC校验，使用查表法，代码来自网络 
    public class ModbusCRC
    {

        #region
        private static readonly byte[] _auchCRCHi = new byte[]//crc高位表
         {
            
         };
        private static readonly byte[] _auchCRCLo = new byte[]//crc低位表
         {
             
         };
        #endregion
        //听说查表法比较省时，确实如此么？
        public static ushort CRC16(Byte[] buffer, int Sset, int Eset)
        {
            byte crcHi = 0xff; // 高位初始化
            byte crcLo = 0xff; // 低位初始化
            for (int i = Sset; i <= Eset; i++)
            {
                int crcIndex = crcHi ^ buffer[i]; //查找crc表值
                crcHi = (byte)(crcLo ^ _auchCRCHi[crcIndex]);
                crcLo = _auchCRCLo[crcIndex];
            }
            return (ushort)(crcHi << 8 | crcLo);
        }
        public static void CRC16(Byte[] buffer, int Sset, int Eset, out byte hiCRCBit, out byte loCRCBit)
        {
            byte crcHi = 0xff; // 高位初始化
            byte crcLo = 0xff; // 低位初始化
            for (int i = Sset; i <= Eset; i++)
            {
                int crcIndex = crcHi ^ buffer[i]; //查找crc表值
                crcHi = (byte)(crcLo ^ _auchCRCHi[crcIndex]);
                crcLo = _auchCRCLo[crcIndex];
            }
            hiCRCBit = crcHi;
            loCRCBit = crcLo;
        }
        //modbus CRC16
        //另一种算法

        public static ushort CRC16Calc(byte[] dataBuff, int dataLen)
        {
            byte CRC16High = 0;
            byte CRC16Low = 0;
            int CRCResult = 0xFFFF;
            for (int i = 0; i < dataLen; i++)
            {
                CRCResult = CRCResult ^ dataBuff[i];
                for (int j = 0; j < 8; j++)
                {
                    if ((CRCResult & 1) == 1)
                        CRCResult = (CRCResult >> 1) ^ 0xA001;
                    else
                        CRCResult >>= 1;
                }
            }
            CRC16High = Convert.ToByte(CRCResult & 0xff);
            CRC16Low = Convert.ToByte(CRCResult >> 8);
            return (ushort)(CRC16High << 8 | CRC16Low);
        }
        public static void CRC16Calc(byte[] dataBuff, int dataLen, out byte hiCRCBit, out byte loCRCBit)
        {
            byte CRC16High = 0;
            byte CRC16Low = 0;
            int CRCResult = 0xFFFF;
            for (int i = 0; i < dataLen; i++)
            {
                CRCResult = CRCResult ^ dataBuff[i];
                for (int j = 0; j < 8; j++)
                {
                    if ((CRCResult & 1) == 1)
                        CRCResult = (CRCResult >> 1) ^ 0xA001;
                    else
                        CRCResult >>= 1;
                }
            }
            CRC16High = Convert.ToByte(CRCResult & 0xff);
            CRC16Low = Convert.ToByte(CRCResult >> 8);
            hiCRCBit = CRC16High;
            loCRCBit = CRC16Low;
        }


    }
}



